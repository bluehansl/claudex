...
async fn fetch_latest_npm_version() -> anyhow::Result<(String, NpmPackageInfo)> {
    let package_info = create_client()
        ...
        .json::<NpmPackageInfo>()
        .await?;
    let latest_version = npm_registry::latest_version(&package_info)?
        .ok_or_else(|| anyhow::anyhow!("npm package is missing latest dist-tag"))?;
    Ok((latest_version, package_info))
}
