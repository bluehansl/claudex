...
//! user's home directory to the current working directory when the current
//! working directory is inside the user's home directory. This is a Claudex
//! extension that lets one host-level AGENTS.md apply before repository-level
//! instructions.
//!
...
//!     that order.
//! 3.  For paths outside the user's home directory, fall back to the upstream
//!     project-root behavior and do **not** walk past the project root.
...
async fn agents_md_paths(
    ...
    let search_dirs = if let Some(home_root) = home_instruction_root(&dir)? {
        search_dirs_from_root_to_dir(home_root, dir)
    } else {
        project_search_dirs(config, dir, fs, sandbox).await?
    };
...
async fn project_search_dirs(
    ...
    if let Some(root) = project_root {
        Ok(search_dirs_from_root_to_dir(root, dir))
    } else {
        Ok(vec![dir])
    }
}

fn search_dirs_from_root_to_dir(root: PathUri, dir: PathUri) -> Vec<PathUri> {
...
fn home_instruction_root(cwd: &PathUri) -> io::Result<Option<PathUri>> {
    let Ok(cwd_abs) = cwd.to_abs_path() else {
        return Ok(None);
    };
    ...
    if cwd_abs.as_path().starts_with(home.as_path()) {
        Ok(Some(PathUri::from_abs_path(&home)))
    } else {
        Ok(None)
    }
}
