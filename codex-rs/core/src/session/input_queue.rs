...
pub(crate) struct InputQueue {
    ...
    mailbox_pending_mails: Mutex<VecDeque<PendingMailboxInput>>,
}

enum PendingMailboxInput {
    Communication {
...
impl PendingMailboxInput {
    fn trigger_turn(&self) -> bool {
        match self {
            Self::Communication { communication, .. } => communication.trigger_turn,
            Self::ResponseItem { trigger_turn, .. } => *trigger_turn,
        }
    }

    fn parent_turn_id(&self) -> Option<&str> {
        match self {
            Self::Communication { parent_turn_id, .. } => parent_turn_id.as_deref(),
            Self::ResponseItem { .. } => None,
        }
    }

    fn root_turn_id(&self) -> Option<&str> {
        match self {
            Self::Communication {
                parent_turn_id,
                root_turn_id,
    ...
    fn into_turn_input(self) -> TurnInput {
        match self {
            ...
            Self::ResponseItem { item, .. } => {
                TurnInput::ResponseItem(ResponseItemEnvelope::new(item))
            }
        }
    }
}
...
impl InputQueue {
    ...
    pub(crate) async fn enqueue_mailbox_communication(
        ...
        self.mailbox_pending_mails
            .lock()
            .await
            .push_back(PendingMailboxInput::Communication {
                communication,
                parent_turn_id,
                root_turn_id,
    ...
    pub(crate) async fn enqueue_mailbox_response_item(
        ...
        self.mailbox_pending_mails
            .lock()
            .await
            .push_back(PendingMailboxInput::ResponseItem {
                item,
                trigger_turn,
                _diagnostics_guard: PENDING_MAILBOX_MESSAGES.track(),
            });
    ...
    pub(crate) async fn has_trigger_turn_mailbox_items(&self) -> bool {
        self.mailbox_pending_mails
            ...
            .await
            .iter()
            .any(PendingMailboxInput::trigger_turn)
    }
    ...
    pub(crate) async fn drain_mailbox_input_items(
        ...
        let parent_turn_id = pending_mails
            ...
            .map(PendingMailboxInput::parent_turn_id)
        ...
        let root_turn_id = pending_mails
            .iter()
            .filter(|mail| mail.trigger_turn())
            .map(PendingMailboxInput::root_turn_id)
            .reduce(|expected, candidate| expected.filter(|id| candidate == Some(*id)))
            .and_then(|id| id.filter(|id| !id.trim().is_empty()).map(str::to_string));
        let items = pending_mails
            .into_iter()
            .map(PendingMailboxInput::into_turn_input)
            .collect();
