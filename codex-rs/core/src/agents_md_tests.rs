...
async fn home_ancestors_are_loaded_before_nested_project_docs() {
    ...
    let mut cfg = make_config(&root, /*limit*/ 1_000_000, /*instructions*/ None).await;
    cfg.cwd = project.abs();

    let discovery = agents_md_paths(&cfg).await.expect("discover paths");
    let expected_tail = vec![
        PathUri::from_abs_path(&root.path().join("AGENTS.md").abs()),
        PathUri::from_abs_path(&org.join("AGENTS.md").abs()),
        PathUri::from_abs_path(&project.join("AGENTS.md").abs()),
    ];
    assert!(
        discovery.ends_with(&expected_tail),
        "expected discovery tail {expected_tail:?}, got {discovery:?}"
    );

    let loaded = get_user_instructions(&cfg).await.expect("doc expected");
    let home_idx = loaded.find("home scoped doc").expect("home scoped doc");
    let org_idx = loaded.find("org doc").expect("org doc");
    let project_idx = loaded.find("project doc").expect("project doc");
    assert!(
        home_idx < org_idx && org_idx < project_idx,
        "expected home -> org -> project order, got {loaded:?}"
    );
}
