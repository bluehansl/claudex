...
impl ToolPlanProbe {
    ...
    fn assert_send_message_uses_plaintext_message(&self) {
        ...
        assert_send_message_tool_uses_plaintext_message(tool);
    }

    fn assert_namespaced_send_message_uses_plaintext_message(&self, namespace: &str) {
        let tool = self
            .visible_specs
            .iter()
            .find_map(|spec| match spec {
                ToolSpec::Namespace(namespace_spec) if namespace_spec.name == namespace => {
                    namespace_spec.tools.iter().find_map(|tool| match tool {
            ...
            .unwrap_or_else(|| {
                panic!(
                    "expected send_message in namespace `{namespace}` within {:?}",
                    self.namespace_functions
                )
            });
        assert_send_message_tool_uses_plaintext_message(tool);
...
fn assert_send_message_tool_uses_plaintext_message(tool: &ResponsesApiTool) {
    let properties = tool
        .parameters
        .properties
        .as_ref()
